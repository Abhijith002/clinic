...
function Calendar({ slots }) {
  ...
  const [activeElement, setActiveElement] = useState(slots[0]);
  const [activeSlot, setActiveSlot] = useState("");
  console.log(new Date(slots[0].date));

  const handleOnDateSelection = (element) => {
  ...
  return (
    <Fragment>
      ...
      <div className="xl:w-5/12 mt-4">
          ...
          {slots.length > 0 &&
            slots.map((element) => {
              return (
                <div
                  key={element.id}
                  onClick={() => handleOnDateSelection(element)}
                >
                  {/* <div
                    className={
                      element.id == activeID
        ...
        {activeElement.slots.length != 0 ? (
          <div className="flex justify-center p-4">
            <div className="bg-white shadow-xl rounded-lg w-3/4">
              <ul className="divide-y divide-gray-300">
                {activeElement.slots.length &&
                  activeElement.slots.map((slot) => {
                    return (
                      <li
      ...
      <UserDetails activeElement={activeElement} activeSlot={activeSlot} />
    </Fragment>
  );
}
